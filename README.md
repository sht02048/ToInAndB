...
하지만, 두 방법 모두 결점이 있기에 더 나은 해결책이 없을까 조사하던 중에 자바스크립트 게임 라이브러리인 **PhaserJS**가 델타 타임 애니메이션이라는 기법을 사용해 프레임을 조절한다는 것을 깨달았습니다. 델타 타임 애니메이션이란 프레임률이 변동해도 일관된 속도와 동작을 유지할 수 있도록 하는 기법입니다. 델타 타임은 이전 프레임과 현재 프레임 사이의 시간 간격을 의미하며, 이를 이용해 애니메이션 또는 게임의 업데이트 주기를 조정합니다.
